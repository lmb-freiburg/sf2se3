...
class Loader(object):
    def __init__(self, args):
        ...
        self.decoder_fea = Decoder(
            alphas=self._alphas_fea, betas=self._betas_fea, steps=self.args.step
        )

        self._alphas_mat = checkpoint["state_dict"]["matching.alphas"]
        self._betas_mat = checkpoint["state_dict"]["matching.betas"]
        self.decoder_mat = Decoder(
            alphas=self._alphas_mat, betas=self._betas_mat, steps=self.args.step
        )
...
def get_new_network_cell():
    ...
    (
        ...
        mat_net_paths,
        mat_net_paths_space,
    ) = load_model.decode_architecture()
