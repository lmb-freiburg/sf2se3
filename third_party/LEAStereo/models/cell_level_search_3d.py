...
class Cell(nn.Module):
    def __init__(
        ...
        prev_fmultiplier_down,
        prev_fmultiplier_same,
        prev_fmultiplier_up,
        filter_multiplier,
    ):
    ...
    def forward(self, s0, s1_down, s1_same, s1_up, n_alphas):
        ...
        if s0 is not None:
            s0 = (
                F.interpolate(
                    s0, (size_d, size_h, size_w), mode="trilinear", align_corners=True
                )
                if (s0.shape[3] != size_h)
                or (s0.shape[4] != size_w)
                or (s0.shape[2] != size_d)
                else s0
            )
