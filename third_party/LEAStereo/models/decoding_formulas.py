...
class Decoder(object):
    def __init__(self, alphas, betas, steps):
        ...
        for layer in range(self._num_layers):
            if layer == 0:
                self.network_space[layer][0][1:] = F.softmax(
                    self._betas[layer][0][1:], dim=-1
                ) * (2 / 3)
            elif layer == 1:
                self.network_space[layer][0][1:] = F.softmax(
                    self._betas[layer][0][1:], dim=-1
                ...
                self.network_space[layer][1] = F.softmax(self._betas[layer][1], dim=-1)
                self.network_space[layer][2] = F.softmax(self._betas[layer][2], dim=-1)
                self.network_space[layer][3][:2] = F.softmax(
                    self._betas[layer][3][:2], dim=-1
                ) * (2 / 3)
    ...
    def viterbi_decode(self):
        ...
        for i in range(1, self._num_layers):
            actual_path[-i - 1] = (
                actual_path[-i] + path_space[self._num_layers - i, actual_path[-i]]
            )
    ...
    def genotype_decode(self):
        def _parse(alphas, steps):
            ...
            for i in range(steps):
                ...
                edges = sorted(
                    range(start, end), key=lambda x: -np.max(alphas[x, 1:])
                )  # ignore none value
                top2edges = edges[:2]
                for j in top2edges:
