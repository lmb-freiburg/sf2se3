...
class AutoMatching(nn.Module):
    ...
    def forward(self, x):
        ...
        upsample_6 = nn.Upsample(
            size=stem.size()[2:], mode="trilinear", align_corners=True
        )
        upsample_12 = nn.Upsample(
            size=[d // 2, h // 2, w // 2], mode="trilinear", align_corners=True
        )
        upsample_24 = nn.Upsample(
            size=[d // 4, h // 4, w // 4], mode="trilinear", align_corners=True
        )
