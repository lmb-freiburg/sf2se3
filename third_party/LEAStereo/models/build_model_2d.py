...
from ..models.cell_level_search_2d import Cell as cell_level_search_2d_Cell
...
class DispEntropy(nn.Module):
    ...
    def forward(self, x):
        x = F.interpolate(
            x,
            [self.maxdisp, x.size()[3] * 3, x.size()[4] * 3],
            mode="trilinear",
            align_corners=False,
        )
        x = torch.squeeze(x, 1)
...
class AutoFeature(nn.Module):
    def __init__(
        ...
        block_multiplier=4,
        step=4,
        cell=cell_level_search_2d_Cell,
    ):
        ...
        self.stem0 = ConvBR(
            3, half_f_initial * self._block_multiplier, 3, stride=1, padding=1
        )
        self.stem1 = ConvBR(
        ...
        self.last_3 = ConvBR(
            self._num_end, self._num_end, 1, 1, 0, bn=False, relu=False
        )
    ...
    def forward(self, x):
        ...
        if torch.cuda.device_count() > 1:
            ...
            for layer in range(len(self.betas)):
                if layer == 0:
                    normalized_betas[layer][0][1:] = F.softmax(
                        self.betas[layer][0][1:].to(device=img_device), dim=-1
                    ) * (2 / 3)
                    ...
                    normalized_betas[layer][3][:2] = F.softmax(
                        self.betas[layer][3][:1].to(device=img_device), dim=-1
                    ) * (2 / 3)
            ...
            for layer in range(len(self.betas)):
                if layer == 0:
                    ...
                    normalized_betas[layer][2] = F.softmax(self.betas[layer][2], dim=-1)
                    normalized_betas[layer][3][:2] = F.softmax(
                        self.betas[layer][3][:2], dim=-1
                    ) * (2 / 3)

        for layer in range(self._num_layers):

            if layer == 0:
                (level3_new,) = self.cells[count](
                    None, None, self.level_3[-1], None, normalized_alphas
                )
                count += 1
                (level6_new,) = self.cells[count](
                    None, self.level_3[-1], None, None, normalized_alphas
                )
                count += 1
                ...
                level3_new_1, level3_new_2 = self.cells[count](
                    self.level_3[-2],
                    None,
                    self.level_3[-1],
                    self.level_6[-1],
                ...
                count += 1
                level3_new = (
                    normalized_betas[layer][0][1] * level3_new_1
                    + normalized_betas[layer][1][0] * level3_new_2
                )

                level6_new_1, level6_new_2 = self.cells[count](
                    None, self.level_3[-1], self.level_6[-1], None, normalized_alphas
                )
                count += 1
                level6_new = (
                    normalized_betas[layer][0][2] * level6_new_1
                    + normalized_betas[layer][1][2] * level6_new_2
                )

                (level12_new,) = self.cells[count](
                    None, self.level_6[-1], None, None, normalized_alphas
                )
                ...
                level6_new_1, level6_new_2, level6_new_3 = self.cells[count](
                    self.level_6[-2],
                    self.level_3[-1],
                    self.level_6[-1],
                    self.level_12[-1],
                ...
                level6_new = (
                    normalized_betas[layer][0][2] * level6_new_1
                    + normalized_betas[layer][1][1] * level6_new_2
                    + normalized_betas[layer][2][0] * level6_new_3
                )

                level12_new_1, level12_new_2 = self.cells[count](
                    None, self.level_6[-1], self.level_12[-1], None, normalized_alphas
                )
                count += 1
                level12_new = (
                    normalized_betas[layer][1][2] * level12_new_1
                    + normalized_betas[layer][2][1] * level12_new_2
                )

                (level24_new,) = self.cells[count](
                    None, self.level_12[-1], None, None, normalized_alphas
                )
                ...
                level12_new_1, level12_new_2, level12_new_3 = self.cells[count](
                    self.level_12[-2],
                    self.level_6[-1],
                    self.level_12[-1],
                    self.level_24[-1],
                ...
                level12_new = (
                    normalized_betas[layer][1][2] * level12_new_1
                    + normalized_betas[layer][2][1] * level12_new_2
                    + normalized_betas[layer][3][0] * level12_new_3
                )

                level24_new_1, level24_new_2 = self.cells[count](
                    None, self.level_12[-1], self.level_24[-1], None, normalized_alphas
                )
                count += 1
                level24_new = (
                    normalized_betas[layer][2][2] * level24_new_1
                    + normalized_betas[layer][3][1] * level24_new_2
                )
        ...
        upsample_6 = nn.Upsample(
            size=stem2.size()[2:], mode="bilinear", align_corners=True
        )
        upsample_12 = nn.Upsample(
            size=[h // 2, w // 2], mode="bilinear", align_corners=True
        )
        upsample_24 = nn.Upsample(
            size=[h // 4, w // 4], mode="bilinear", align_corners=True
        )

        result_3 = self.last_3(self.level_3[-1])
        result_6 = self.last_3(upsample_6(self.last_6(self.level_6[-1])))
        result_12 = self.last_3(
            upsample_6(self.last_6(upsample_12(self.last_12(self.level_12[-1]))))
        )
        result_24 = self.last_3(
            upsample_6(
                self.last_6(upsample_12(self.last_12(self.last_24(self.level_24[-1]))))
            )
        )
    ...
    def _initialize_alphas_betas(self):
        ...
        betas = (
            (1e-3 * torch.randn(self._num_layers, 4, 3))
            .clone()
            .detach()
            .requires_grad_(True)
        )
    ...
    def arch_parameters(self):
        return [
            ...
            if name in self._arch_param_names
        ]
    ...
    def weight_parameters(self):
        return [
            param
            for name, param in self.named_parameters()
            if name not in self._arch_param_names
        ]
