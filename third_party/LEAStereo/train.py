...
scheduler = optim.lr_scheduler.MultiStepLR(
    optimizer, milestones=opt.milestones, gamma=0.5
)
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        if valid > 0:
            ...
            print(
                "===> Epoch[{}]({}/{}): Loss: ({:.4f}), Error: ({:.4f}), Time: ({:.2f}s)".format(
                    epoch,
                    iteration,
            ...
            sys.stdout.flush()
    print(
        "===> Epoch {} Complete: Avg. Loss: ({:.4f}), Avg. Error: ({:.4f})".format(
            epoch, epoch_loss / valid_iteration, epoch_error / valid_iteration
        )
    )
    ...
    for iteration, batch in enumerate(testing_data_loader):
        ...
        if valid > 0:
            with torch.no_grad():
                ...
                print(
                    "===> Test({}/{}): Error: ({:.4f} {:.4f})".format(
                        iteration, len(testing_data_loader), error.item(), three_px_acc
                    )
                )
                sys.stdout.flush()
