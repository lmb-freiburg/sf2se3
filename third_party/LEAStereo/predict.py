...
mult_adds = comp_multadds(
    model, input_size=(3, opt.crop_height, opt.crop_width)
)  # (3,192, 192))
...
if __name__ == "__main__":
    ...
    for index in range(len(filelist)):
        ...
        if opt.middlebury:
            ...
            temppath = opt.save_path.replace(
                opt.save_path.split("/")[-2], opt.save_path.split("/")[-2] + "/images"
            )
