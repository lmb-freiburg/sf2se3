...
class Cell(nn.Module):
    ...
    def forward(self, prev_prev_input, prev_input):
        ...
        if self.downup_sample != 0:
            ...
            s1 = F.interpolate(
                s1,
                [feature_size_d, feature_size_h, feature_size_w],
                mode="trilinear",
                align_corners=True,
            )
        if (
            (s0.shape[2] != s1.shape[2])
            or (s0.shape[3] != s1.shape[3])
            or (s0.shape[4] != s1.shape[4])
        ):
            s0 = F.interpolate(
                s0,
                (s1.shape[2], s1.shape[3], s1.shape[4]),
                mode="trilinear",
                align_corners=True,
            )
...
class newMatching(nn.Module):
    ...
    def forward(self, x):
        ...
        upsample_6 = nn.Upsample(
            size=x.size()[2:], mode="trilinear", align_corners=True
        )
        ...
        if last_output.size()[3] == h:
            ...
            mat = self.last_3(
                upsample_6(self.last_6(upsample_12(self.last_12(last_output))))
            )
            ...
            mat = self.last_3(
                upsample_6(
                    self.last_6(
                        upsample_12(
                            self.last_12(upsample_24(self.last_24(last_output)))
                        )
                    )
                )
            )
