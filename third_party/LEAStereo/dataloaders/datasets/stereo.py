...
def train_transform(temp_data, crop_height, crop_width, left_right=False, shift=0):
    ...
    if h > crop_height and w <= crop_width:
        ...
        temp_data[6:7, :, :] = 1000
        temp_data[
            :, h + shift - h : h + shift, crop_width + shift - w : crop_width + shift
        ] = temp
        _, h, w = np.shape(temp_data)
    ...
    if h <= crop_height and w <= crop_width:
        ...
        temp_data[
            :,
            crop_height + shift - h : crop_height + shift,
            crop_width + shift - w : crop_width + shift,
        ] = temp
        _, h, w = np.shape(temp_data)
    if shift > 0:
        ...
        right = temp_data[
            3:6, start_y : start_y + crop_height, start_x : start_x + crop_width
        ]
        target = temp_data[
            6:7,
            start_y : start_y + crop_height,
...
def load_kitti2012_data(file_path, current_file):
    ...
    right = Image.open(filename)
    filename = (
        file_path + "disp_noc/" + current_file[0 : len(current_file) - 1]
    )  # disp_occ
...
class DatasetFromList(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.args.dataset == "kitti12":  # load kitti2012 dataset
            temp_data = load_kitti2012_data(
                Path.db_root_dir("kitti12"), self.file_list[index]
            )
        elif self.args.dataset == "kitti15":  # load kitti2015 dataset
            temp_data = load_kitti2015_data(
                Path.db_root_dir("kitti15"), self.file_list[index]
            )
        elif self.args.dataset == "sceneflow":  # load sceneflow dataset
            temp_data = load_data_sceneflow(
                Path.db_root_dir("sceneflow"), self.file_list[index]
            )
        elif self.args.dataset == "middlebury":  # load middbury dataset
            temp_data = load_data_md(
