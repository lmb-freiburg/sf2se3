...
def make_data_loader(args, **kwargs):
    ...
    if args.dataset == "sceneflow":
        ...
        train_list = (
            "dataloaders/lists/sceneflow_train.list"  # original training set: 35,454
        )
        test_list = "dataloaders/lists/sceneflow_test.list"  # original test set:4,370
        trainA_set = stereo.DatasetFromList(
            args, trainA_list, [args.crop_height, args.crop_width], True
        )
        trainB_set = stereo.DatasetFromList(
            args, trainB_list, [args.crop_height, args.crop_width], True
        )
        train_set = stereo.DatasetFromList(
            args, train_list, [args.crop_height, args.crop_width], True
        )
        ...
        if args.stage == "search":
            train_loaderA = DataLoader(
                trainA_set, batch_size=args.batch_size, shuffle=True, **kwargs
            )
            train_loaderB = DataLoader(
                trainB_set, batch_size=args.batch_size, shuffle=True, **kwargs
            )
        ...
        val_loader = DataLoader(
            val_set, batch_size=args.testBatchSize, shuffle=False, **kwargs
        )
        test_loader = DataLoader(
            test_set, batch_size=args.testBatchSize, shuffle=False, **kwargs
        )
