...
import torch
from m4_io.m4 import dtype_torch2numpy
...
def readCameraData(cameradata_fp, dtype=torch.float32):
    file = open(cameradata_fp, "rb")

    dtype = dtype_torch2numpy(dtype)
    frames_ids = []
    transfs_left = []
    transfs_right = []

    with open(cameradata_fp, "r") as f:
        for line in f.readlines():
            if line.startswith("Frame"):
                frames_ids.append(int(line.split(" ")[1].strip()))
            elif line.startswith("L"):
                transfs_left.append(
                    torch.from_numpy(
                        np.fromstring(line.rstrip()[2:], dtype=dtype, sep=" ")
                    ).reshape(4, 4)
                )
            elif line.startswith("R"):
                transfs_right.append(
                    torch.from_numpy(
                        np.fromstring(line.rstrip()[2:], dtype=dtype, sep=" ")
                    ).reshape(4, 4)
                )

    return frames_ids, transfs_left, transfs_right
